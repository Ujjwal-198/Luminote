...
const LabManual = () => {
    ...
    React.useEffect(() => {
        dispatch(getDocuments({ category: 'Lab Manuals' }));
    }, [dispatch]);
    ...
    const allLabManuals = documents.map(doc => ({
        id: doc._id,
        title: doc.title,
        branch: doc.branch,
    ...
    const filteredLabManuals = allLabManuals.filter(manual =>
        branchMatches(manual.branch, branchFilter) &&
        (academicSessionFilter === 'All' || manual.academicSession === academicSessionFilter) &&
        (subjectFilter === 'All' || manual.subject === subjectFilter) &&
        (searchTerm === '' ||
            manual.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
            manual.subject.toLowerCase().includes(searchTerm.toLowerCase()) ||
            manual.description?.toLowerCase().includes(searchTerm.toLowerCase())
        )
    );

    const [currentPage, setCurrentPage] = useState(1);
    const manualsPerPage = parseInt(import.meta.env.VITE_DOCUMENTS_PER_PAGE) || 9;
    const totalPages = Math.ceil(filteredLabManuals.length / manualsPerPage);
    const startIndex = (currentPage - 1) * manualsPerPage;
    const paginatedManuals = filteredLabManuals.slice(startIndex, startIndex + manualsPerPage);
        ...
        <div className='flex flex-col align-middle justify-start p-10 bg-zinc-950 min-h-screen w-full'>
            <div>
                <SearchAndFilter
                    ...
                    searchPlaceholder="Search Lab Manuals"
                ...
                <div className='flex align-middle justify-center items-center w-full'>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-center align-middle justify-center px-2 py-10 md:px-10 w-full'>
                        {paginatedManuals.map((manual) => (
                            <DocumentCard className='bg-zinc-800' key={manual.id} file={manual} />
                        ))}
                    </div>
                </div>
            )}

            {filteredLabManuals.length > manualsPerPage && <Pagination
                pages={totalPages}
                page={currentPage}
            ...
            {filteredLabManuals.length > 0 && (
                <div className="text-center text-gray-400 text-sm mt-4">
                    Showing {startIndex + 1}-{Math.min(startIndex + manualsPerPage, filteredLabManuals.length)} of {filteredLabManuals.length} lab manuals
                </div>
            )}
        </div>
    );
