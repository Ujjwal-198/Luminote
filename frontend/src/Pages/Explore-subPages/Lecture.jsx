...
const Lecture = () => {
    ...
    React.useEffect(() => {
        dispatch(getDocuments({ category: 'Lecture' }));
    }, [dispatch]);
    ...
    const allLectures = documents.map(doc => ({
        id: doc._id,
        title: doc.title,
        branch: doc.branch,
    ...
    const filteredLectures = allLectures.filter(lecture =>
        branchMatches(lecture.branch, branchFilter) &&
        (academicSessionFilter === 'All' || lecture.academicSession === academicSessionFilter) &&
        (subjectFilter === 'All' || lecture.subject === subjectFilter) &&
        (searchTerm === '' ||
    ...
    const lecturesPerPage = parseInt(import.meta.env.VITE_DOCUMENTS_PER_PAGE) || 9;
    ...
    return (
        <div className='flex flex-col align-middle justify-start p-10 bg-zinc-950 min-h-screen w-full'>
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-center align-middle justify-center px-2 py-10 md:px-10 w-full'>
                        {
                            paginatedLectures.map((lecture) => (
                                <DocumentCard className='bg-zinc-800' key={lecture.id} file={lecture} />
                            ))
                        }
                    </div>
                </div>
            ...
            {filteredLectures.length > lecturesPerPage && <Pagination
                pages={totalPages}
                page={currentPage}
