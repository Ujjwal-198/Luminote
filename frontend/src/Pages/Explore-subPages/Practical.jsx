...
const Practical = () => {
    ...
    React.useEffect(() => {
        dispatch(getDocuments({ category: 'Practical' }));
    }, [dispatch]);
    ...
    const allPractical = documents.map(doc => ({
        id: doc._id,
        title: doc.title,
        branch: doc.branch,
    ...
    const filteredPractical = allPractical.filter(practical =>
        branchMatches(practical.branch, branchFilter) &&
        (academicSessionFilter === 'All' || practical.academicSession === academicSessionFilter) &&
        (subjectFilter === 'All' || practical.subject === subjectFilter) &&
        (searchTerm === '' ||
            practical.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
            practical.subject.toLowerCase().includes(searchTerm.toLowerCase()) ||
            practical.description?.toLowerCase().includes(searchTerm.toLowerCase())
        )
    );

    const [currentPage, setCurrentPage] = useState(1);
    const practicalPerPage = parseInt(import.meta.env.VITE_DOCUMENTS_PER_PAGE) || 9;
    const totalPages = Math.ceil(filteredPractical.length / practicalPerPage);
    const startIndex = (currentPage - 1) * practicalPerPage;
    const endIndex = startIndex + practicalPerPage;
    const paginatedPractical = filteredPractical.slice(startIndex, endIndex);
        ...
        <div className='flex flex-col align-middle justify-start p-10 bg-zinc-950 min-h-screen w-full'>
            <div>
                <SearchAndFilter
                    ...
                    searchPlaceholder="Search Practical Work"
            ...
            {loading ? (
                ...
                <div className='flex align-middle justify-center items-center w-full'>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-center align-middle justify-center px-2 py-10 md:px-10 w-full'>
                        {
                            paginatedPractical.map((practical) => (
                                <DocumentCard className='bg-zinc-800' key={practical.id} file={practical} />
                            ))
                        }
                    </div>
                </div>
            ...
            {filteredPractical.length > practicalPerPage && <Pagination
                pages={totalPages}
                page={currentPage}
            ...
            {filteredPractical.length > 0 && (
                <div className="text-center text-gray-400 text-sm mt-4">
                    Showing {startIndex + 1}-{Math.min(endIndex, filteredPractical.length)} of {filteredPractical.length} practical work
                </div>
            )}
        </div>
    );
