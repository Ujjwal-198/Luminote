...
const Practice = () => {
    ...
    React.useEffect(() => {
        dispatch(getDocuments({ category: 'Practice materials' }));
    }, [dispatch]);
    ...
    const allPractice = documents.map(doc => ({
        id: doc._id,
        title: doc.title,
        branch: doc.branch,
    ...
    const filteredPractice = allPractice.filter(practice =>
        branchMatches(practice.branch, branchFilter) &&
        (academicSessionFilter === 'All' || practice.academicSession === academicSessionFilter) &&
        (subjectFilter === 'All' || practice.subject === subjectFilter) &&
        (searchTerm === '' ||
            practice.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
            practice.subject.toLowerCase().includes(searchTerm.toLowerCase()) ||
            practice.description?.toLowerCase().includes(searchTerm.toLowerCase())
        )
    );

    const [currentPage, setCurrentPage] = useState(1);
    const practicePerPage = parseInt(import.meta.env.VITE_DOCUMENTS_PER_PAGE) || 9;
    const totalPages = Math.ceil(filteredPractice.length / practicePerPage);
    const startIndex = (currentPage - 1) * practicePerPage;
    const endIndex = startIndex + practicePerPage;
    const paginatedPractice = filteredPractice.slice(startIndex, endIndex);
        ...
        <div className='flex flex-col align-middle justify-start p-10 bg-zinc-950 min-h-screen w-full'>
            <div>
                <SearchAndFilter
                    ...
                    searchPlaceholder="Search Practice Materials"
            ...
            {loading ? (
                ...
                <div className='flex align-middle justify-center items-center w-full'>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-center align-middle justify-center px-2 py-10 md:px-10 w-full'>
                        {
                            paginatedPractice.map((practice) => (
                                <DocumentCard className='bg-zinc-800' key={practice.id} file={practice} />
                            ))
                        }
                    </div>
                </div>
            ...
            {filteredPractice.length > practicePerPage && <Pagination
                pages={totalPages}
                page={currentPage}
            ...
            {filteredPractice.length > 0 && (
                <div className="text-center text-gray-400 text-sm mt-4">
                    Showing {startIndex + 1}-{Math.min(endIndex, filteredPractice.length)} of {filteredPractice.length} practice materials
                </div>
            )}
        </div>
    );
