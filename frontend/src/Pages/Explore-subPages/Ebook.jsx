...
const Ebook = () => {
    ...
    React.useEffect(() => {
        dispatch(getDocuments({ category: 'Reference books' }));
    }, [dispatch]);
    ...
    const allEbooks = documents.map(doc => ({
        id: doc._id,
        title: doc.title,
        branch: doc.branch,
    ...
    const filteredEbooks = allEbooks.filter(ebook =>
        branchMatches(ebook.branch, branchFilter) &&
        (academicSessionFilter === 'All' || ebook.academicSession === academicSessionFilter) &&
        (subjectFilter === 'All' || ebook.subject === subjectFilter) &&
        (searchTerm === '' ||
            ebook.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ebook.subject.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ebook.description?.toLowerCase().includes(searchTerm.toLowerCase())
        )
    );

    const [currentPage, setCurrentPage] = useState(1);
    const ebooksPerPage = parseInt(import.meta.env.VITE_DOCUMENTS_PER_PAGE) || 9;
    const totalPages = Math.ceil(filteredEbooks.length / ebooksPerPage);
    const startIndex = (currentPage - 1) * ebooksPerPage;
    const paginatedEbooks = filteredEbooks.slice(startIndex, startIndex + ebooksPerPage);
        ...
        <div className='flex flex-col align-middle justify-start p-10 bg-zinc-950 min-h-screen w-full'>
                ...
                <div className='flex align-middle justify-center items-center w-full'>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-center align-middle justify-center px-2 py-10 md:px-10 w-full'>
                        {paginatedEbooks.map((ebook) => (
                            <DocumentCard className='bg-zinc-800' key={ebook.id} file={ebook} />
                        ))}
                    </div>
                </div>
            )}

            {filteredEbooks.length > ebooksPerPage && <Pagination
                pages={totalPages}
                page={currentPage}
            ...
            {filteredEbooks.length > 0 && (
                <div className="text-center text-gray-400 text-sm mt-4">
                    Showing {startIndex + 1}-{Math.min(startIndex + ebooksPerPage, filteredEbooks.length)} of {filteredEbooks.length} e-books
                </div>
            )}
        </div>
    );
