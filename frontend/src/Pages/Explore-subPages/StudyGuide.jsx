...
const StudyGuide = () => {
    ...
    React.useEffect(() => {
        dispatch(getDocuments({ category: 'Summaries' }));
    }, [dispatch]);
    ...
    const allStudyGuides = documents.map(doc => ({
        id: doc._id,
        title: doc.title,
        branch: doc.branch,
    ...
    const filteredStudyGuides = allStudyGuides.filter(guide =>
        branchMatches(guide.branch, branchFilter) &&
        (academicSessionFilter === 'All' || guide.academicSession === academicSessionFilter) &&
        (subjectFilter === 'All' || guide.subject === subjectFilter) &&
        (searchTerm === '' ||
    ...
    const guidesPerPage = parseInt(import.meta.env.VITE_DOCUMENTS_PER_PAGE) || 9;
    const totalPages = Math.ceil(filteredStudyGuides.length / guidesPerPage);
    const startIndex = (currentPage - 1) * guidesPerPage;
    const paginatedGuides = filteredStudyGuides.slice(startIndex, startIndex + guidesPerPage);
        ...
        <div className='flex flex-col align-middle justify-start p-10 bg-zinc-950 min-h-screen w-full'>
                ...
                <div className='flex align-middle justify-center items-center w-full'>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-center align-middle justify-center px-2 py-10 md:px-10 w-full'>
                        {paginatedGuides.map((guide) => (
                            <DocumentCard className='bg-zinc-800' key={guide.id} file={guide} />
                        ))}
                    </div>
                </div>
            )}

            {filteredStudyGuides.length > guidesPerPage && <Pagination
                pages={totalPages}
                page={currentPage}
            ...
            {filteredStudyGuides.length > 0 && (
                <div className="text-center text-gray-400 text-sm mt-4">
                    Showing {startIndex + 1}-{Math.min(startIndex + guidesPerPage, filteredStudyGuides.length)} of {filteredStudyGuides.length} study guides
                </div>
            )}
        </div>
    );
