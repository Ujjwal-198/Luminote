...
const Essays = () => {
    ...
    React.useEffect(() => {
        dispatch(getDocuments({ category: 'Essays' }));
    }, [dispatch]);
    ...
    const allEssays = documents.map(doc => ({
        id: doc._id,
        title: doc.title,
        branch: doc.branch,
    ...
    const filteredEssays = allEssays.filter(essay =>
        branchMatches(essay.branch, branchFilter) &&
        (academicSessionFilter === 'All' || essay.academicSession === academicSessionFilter) &&
        (subjectFilter === 'All' || essay.subject === subjectFilter) &&
        (searchTerm === '' ||
            essay.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
            essay.subject.toLowerCase().includes(searchTerm.toLowerCase()) ||
            essay.description?.toLowerCase().includes(searchTerm.toLowerCase())
        )
    );

    const [currentPage, setCurrentPage] = useState(1);
    const essaysPerPage = parseInt(import.meta.env.VITE_DOCUMENTS_PER_PAGE) || 9;
    ...
    const paginatedEssays = filteredEssays.slice(startIndex, endIndex);
        ...
        <div className='flex flex-col align-middle justify-start p-10 bg-zinc-950 min-h-screen w-full'>
            <div>
                <SearchAndFilter
                    ...
                    searchPlaceholder="Search Essays & Reports"
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-center align-middle justify-center px-2 py-10 md:px-10 w-full'>
                        {
                            paginatedEssays.map((essay) => (
                                <DocumentCard className='bg-zinc-800' key={essay.id} file={essay} />
                            ))
                        }
                    </div>
                </div>
            ...
            {filteredEssays.length > essaysPerPage && <Pagination
                pages={totalPages}
                page={currentPage}
            ...
            {filteredEssays.length > 0 && (
                <div className="text-center text-gray-400 text-sm mt-4">
                    Showing {startIndex + 1}-{Math.min(endIndex, filteredEssays.length)} of {filteredEssays.length} essays & reports
                </div>
            )}
        </div>
    );
