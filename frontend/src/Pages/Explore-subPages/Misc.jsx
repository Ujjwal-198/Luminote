...
const Misc = () => {
    ...
    React.useEffect(() => {
        dispatch(getDocuments({ category: 'Other' }));
    }, [dispatch]);
    ...
    const allMiscDocs = documents.map(doc => ({
        id: doc._id,
        title: doc.title,
        branch: doc.branch,
    ...
    const filteredMiscDocs = allMiscDocs.filter(doc =>
        branchMatches(doc.branch, branchFilter) &&
        (academicSessionFilter === 'All' || doc.academicSession === academicSessionFilter) &&
        (subjectFilter === 'All' || doc.subject === subjectFilter) &&
        (searchTerm === '' ||
    ...
    const docsPerPage = parseInt(import.meta.env.VITE_DOCUMENTS_PER_PAGE) || 9;
    const totalPages = Math.ceil(filteredMiscDocs.length / docsPerPage);
    const startIndex = (currentPage - 1) * docsPerPage;
    const paginatedDocs = filteredMiscDocs.slice(startIndex, startIndex + docsPerPage);
        ...
        <div className='flex flex-col align-middle justify-start p-10 bg-zinc-950 min-h-screen w-full'>
                ...
                <div className='flex align-middle justify-center items-center w-full'>
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-center align-middle justify-center px-2 py-10 md:px-10 w-full'>
                        {paginatedDocs.map((doc) => (
                            <DocumentCard className='bg-zinc-800' key={doc.id} file={doc} />
                        ))}
                    </div>
                </div>
            )}

            {filteredMiscDocs.length > docsPerPage && <Pagination
                pages={totalPages}
                page={currentPage}
            ...
            {filteredMiscDocs.length > 0 && (
                <div className="text-center text-gray-400 text-sm mt-4">
                    Showing {startIndex + 1}-{Math.min(startIndex + docsPerPage, filteredMiscDocs.length)} of {filteredMiscDocs.length} documents
                </div>
            )}
        </div>
    );
