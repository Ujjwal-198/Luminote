...
const Slides = () => {
    ...
    React.useEffect(() => {
        dispatch(getDocuments({ category: ['Slides', 'PPTs'] }));
    }, [dispatch]);
    ...
    const allSlides = documents.map(doc => ({
        id: doc._id,
        title: doc.title,
        branch: doc.branch,
    ...
    const filteredSlides = allSlides.filter(slide =>
        branchMatches(slide.branch, branchFilter) &&
        (academicSessionFilter === 'All' || slide.academicSession === academicSessionFilter) &&
        (subjectFilter === 'All' || slide.subject === subjectFilter) &&
        (searchTerm === '' ||
    ...
    const slidesPerPage = parseInt(import.meta.env.VITE_DOCUMENTS_PER_PAGE) || 9;
        ...
        <div className='flex flex-col align-middle justify-start p-10 bg-zinc-950 min-h-screen w-full'>
            <div>
                <SearchAndFilter
                    ...
                    searchPlaceholder="Search Slides & PPTs"
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-center align-middle justify-center px-2 py-10 md:px-10 w-full'>
                        {
                            paginatedSlides.map((slide) => (
                                <DocumentCard className='bg-zinc-800' key={slide.id} file={slide} />
                            ))
                        }
                    </div>
                </div>
            ...
            {filteredSlides.length > slidesPerPage && <Pagination
                pages={totalPages}
                page={currentPage}
            ...
            {filteredSlides.length > 0 && (
                <div className="text-center text-gray-400 text-sm mt-4">
                    Showing {startIndex + 1}-{Math.min(endIndex, filteredSlides.length)} of {filteredSlides.length} slides
                </div>
            )}
        </div>
    );
